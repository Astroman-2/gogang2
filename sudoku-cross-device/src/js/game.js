// game.js — pure game state, no DOM
const Game = (() => {
  let S = null;

  function init({ puzzle, solution, mode, players, diff }) {
    S = {
      mode, diff,
      ...
      players: players.map((p,i) => ({
        ...p, score:0, errors:0, moves:0, hints:3, done:false
      })),
      notesMode: false,
      selected: null,
      elapsed: 0,
  ...
  function startTimer(cb) {
    ...
    S._timerRef = setInterval(() => {
      S.elapsed = Math.floor((Date.now() - start) / 1000);
      cb && cb(S.elapsed);
    }, 500);
  }
  ...
  function place(r, c, n, pi) {
    ...
    let correct = false, wasError = false;
    if (n !== 0) {
      p.moves++;
      correct = (n === S.solution[r][c]);
      if (correct) { p.score += S.elapsed < 120 ? 12 : 10; }
    ...
    if (solved && !S.finished) {
      S.finished = true; p.done = true; p.finishTime = S.elapsed;
      stopTimer();
    }
    return { ok:true, type:'place', r, c, n, correct, wasError, solved, pi };
  }
  ...
  function hint(pi) {
    ...
    S.board[r][c] = val; S.owner[r][c] = pi;
    S.notes[r][c].clear(); _clearRelatedNotes(r,c,val);
    p.hints--; p.score += 5;
    const solved = Generator.isSolved(S.board, S.solution);
    if (solved && !S.finished) { S.finished=true; p.done=true; p.finishTime=S.elapsed; stopTimer(); }
    return { r, c, val, solved, hintsLeft: p.hints };
  }
  ...
  function progress(pi = null) {
    ...
    for(let r=0;r<9;r++) for(let c=0;c<9;c++) {
      if (!S.given[r][c]) {
        total++;
        if (S.board[r][c]!==0 && (pi===null||S.owner[r][c]===pi)) filled++;
      }
    }
